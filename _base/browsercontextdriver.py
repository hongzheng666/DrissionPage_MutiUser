...
class Driver(object):
    def __init__(self, tab_id,context_id, tab_type, address, owner=None):
        self.id = tab_id
    ...
    def _handle_event_loop(self):
        while self.is_running:
            ...
            functions = self.event_handlers.get(event['method'])
            if functions:
                '''
                每个browserContext都要处理消息
                '''
                for f in functions:
                    f(**event['params'])
    ...
    def set_callback(self, event, callback, immediate=False):
        ...
        if callback:
            handler[event] =(handler[event] if event in handler else []) + [callback]
        else:
            handler.pop(event, None)


class BrowserContextDriver(Driver):
    BROWSERS = {}

    def __new__(cls, tab_id,context_id, tab_type, address, owner):
        if context_id in cls.BROWSERS:
            return cls.BROWSERS[context_id]
        return object.__new__(cls)

    def __init__(self, tab_id,context_id, tab_type, address, owner):
        if hasattr(self, '_created'):
            return
        self._created = True
        BrowserContextDriver.BROWSERS[context_id] = self
        super().__init__(tab_id,context_id, tab_type, address, owner)
